...
- at execution time this maps to the Service Bus dispatch action handled by `FEAT-EXECUTION-001 Start/Stop Services`
...
- **Acceptance criteria (AC-ENV-014):** Stages with a `latestExecution.status` of `failed` or `partially_failed` are visually highlighted in the Environment Details UI using a subtle attention style (`ring-2 ring-red-300/20 bg-red-50`).
- **Validation test:** A frontend unit test was added at `frontend/src/features/environment/__tests__/EnvironmentDetailsPage.test.tsx` that mocks `GET /api/environments/{id}` and asserts the stage card contains the highlight classes when `latestExecution.status === 'failed'`.
- **Backend contract note:** Server-side augmentation attaches `latestExecution` (and `executionCount`) to stage objects returned by `GET /api/environments/{id}` via the existing augmentation helper. Ensure the augmentation continues to provide `latestExecution.status` for clients that render the highlight.

Add a short validation report referencing the test artifact when creating the release/PR that implements this feature.
