...
Feature ID: FEAT-ENVIRONMENTS-001
...
- `StageResourceAction`
  ...
  - executed at runtime by `FEAT-EXECUTION-001 Start/Stop Services`
- `Schedule`
...
- `FEAT-EXECUTION-001 Start/Stop Services` for execution semantics of the configured stage resource actions

### AC-ENV-014

The environment details page visually highlights stage and schedule cards when the latest execution status is `failed` or `partially_failed`, while preserving the existing execution status badges and keeping the attention styling subtle.
