...
- Frontend environment details page now highlights stage and schedule cards when the latest execution status is `failed` or `partially_failed`
...
- Absorbed enhancement history:
  - the narrow `highlight failed executions on environment details` spec has been folded into this governing environments-management package
